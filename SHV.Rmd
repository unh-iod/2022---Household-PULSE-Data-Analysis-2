...
$$\frac{ds}{dt} = -\beta S + \alpha p V S$$

$$\frac{dh}{dt} = \gamma H + \alpha (1-p) V S$$

$$\frac{dv}{dt} = \beta S -\gamma H - \alpha V S$$
...
s_1 <- function(beta, gamma, alpha, p = p, S0, H0, V0, times) {
  ...
  with(as.list(c(variables, parameters)), {
    dS <- - beta * S + alpha*p * V * S
    dH <- + gamma * H + alpha*(1-p) * V * S
    dV <- + beta * S - gamma * H - alpha * V * S
...
starting_param_val <- c(0.001, 0.001, 0.001, 0.5, 0.20)
